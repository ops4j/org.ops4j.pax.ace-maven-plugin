...
/**
 * Remove link(s) between ACE entries
 *
 * @author dpishchukhin
 */
@Mojo(name = "unlink", requiresProject = false, aggregator = true)
public class UnlinkMojo extends AbstractAceMojo {
     ...
     */
    @Parameter(property = "ace.unlink.type", required = true)
    protected String type;
    /**
     * Left endpoint filter
     */
    @Parameter(property = "ace.unlink.left-endpoint.filter", required = true)
    protected String leftFilter;
    /**
     * Right endpoint filter
     */
    @Parameter(property = "ace.unlink.right-endpoint.filter", required = true)
    ...
    public void internalExecute() throws MojoExecutionException, MojoFailureException {
        ...
        try {
            ...
            try {
                ...
                if (ARTIFACT_2_FEATURE_TYPE.equalsIgnoreCase(type)) {
                    deleteIds = aceRestClient.deleteArtifactDFeatureLinks(leftFilter, rightFilter);
                } else if (FEATURE_2_DISTRIBUTION_TYPE.equalsIgnoreCase(type)) {
                    deleteIds = aceRestClient.deleteFeature2DistributionLinks(leftFilter, rightFilter);
                } else if (DISTRIBUTION_2_TARGET_TYPE.equalsIgnoreCase(type)) {
                    deleteIds = aceRestClient.deleteDistribution2TargetLinks(leftFilter, rightFilter);
                }
                aceRestClient.persist();
                getLog().info(String.format("Links are deletes: %s", deleteIds));
            } finally {
                aceRestClient.close();
            }
        } catch (Exception e) {
            throw new MojoExecutionException(String.format("Unable to remove links in ACE Server"), e);
        }
    }
}
